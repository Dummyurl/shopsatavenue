...
(function(window, $, undefined){
	...
	plugin.dev_skinned = {
		...
		buildCallback: function($elm) {
			...
			$elm.append('<div class="cp-patch"><div></div></div><div class="cp-disp"></div><div><input type="button" onClick="$(\'.cp-color-picker\').hide();" name="apply" value="Ok" style="float:right; color:#000;" ></div>');
		...
		renderCallback: function($elm, toggled) {
			...
			$( $elm ).find('input[name=color_code\\[\\]]').val( $elm[0].style.backgroundColor );
			//$('.cp-disp').text(this.color.options.colorNames[colors.HEX] || $elm.val());
			//$('#color_val').css({'background-color': $elm[0].style.backgroundColor});
			...
			if (toggled === true) {
				...
				//alert( $elm[0].style.backgroundColor );
			}
		}
	};
