...
function unfeature_prod(seourl,page,evt){
	if(seourl != "" && page != ""){
	$.ajax({
		type:'post',
		url:baseURL+'admin/product/unfeaturePro_page',
		data:{'seourl':seourl,'page':page},
		success : function(response){
			if(response != "")
			{
				/* $(evt).remove();
				if(page == "home")
					$('.tbl_row_home').append('<td> <input type="radio" class="required small tipTop" title="Please select the Page"  name="Page" id="Page" value="home">Home page</td>');
				else if(page == "search")
					$('.tbl_row_search').append('<td><input type="radio" class="required small tipTop" title="Please select the Page"  name="Page" id="Page" value="search" >Search page</td>');
				else if(page == "product detail")
					$('.tbl_row_detail').append('<td ><input type="radio" class="required small tipTop" title="Please select the Page"  name="Page" id="Page" value="product detail"  >Product Detail page</td>');
				 */
				 window.location.href = "admin/product/display_product_list";
			}
		}
...
function ajax_getAPIKey(){
	...
	if(username != '' && password != ''){
		...
		{
			alert('password must have 8 or more character');    
			return false;
		}else{
			//alert("ss"); return false;
			$.confirm({
			'title'		: 'Confirmation',
			'message'	: 'Do you wanna Change The API Key ! Continue?',
			'buttons'	: {
				'Yes'	: {
					'class'	: 'yes',
					'action': function(){
...
function ContactSeller(){
	...
	if(question ==''){
		$('#div_question').html(lg_required_field);
		return false;
	}else if(name ==''){
		$('#div_name').html(lg_required_field);
		return false;		
	}else if(email ==''){
		$('#div_emailaddress').html(lg_required_field);
		return false;		
...
function Addshipping(){
	...
	if(country==0) { 
		$('#country_Err').html(lg_required_field);	
	}else if(full_name==0){
			$('#full_name_Err').html(lg_required_field);	
	
	}else if(nick_name==0){
			$('#nick_name_Err').html(lg_required_field);	
	
	}else if(address1==0){
			$('#address1_Err').html(lg_required_field);	
	
	}else if(city==0){
			$('#city_Err').html(lg_required_field);	
	
	}else if(state==0){
			$('#state_Err').html(lg_required_field);	
	
	}else if(!$.isNumeric(postal_code)){
			$('#postal_code_Err').html(lg_required_field);	
	
	}else if(!$.isNumeric(phone)){
			$('#phone_Err').html(lg_required_field);	
...
function AddFeedback(){
	...
	if(title=='') { 
		...
		$('#title_Err').html(lg_required_field);	
	}else {
		var brand = 'no';
		if($('.brandSt').is(':checked')){
			brand = 'yes';
...
function validateSeller_Signup(){
      ...
      //  var pattern = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
		...
		$('#seller_crafting_Err').html(lg_required_field);	
		}else if(seller_product==0){ 
		$('#seller_product_Err').html(lg_required_field);	
		}else if(seller_make==0){ 
		$('#seller_make_Err').html(lg_required_field);	
		}/*else if(!pattern.test(seller_site)){ 
		$('#seller_site_Err').html('This field required');	
		}*/else if(seller_nda==undefined){ 
		$('#seller_nda_Err').html(lg_required_field);	
		}else if(seller_agreement==undefined){ 
		$('#seller_agreement_Err').html(lg_required_field);	
	}else {
		var brand = 'no';
		if($('.brandSt').is(':checked')){
			brand = 'yes';
	        ...
	        success: function(response){
	        	if(response.success == '0') {
					...
					return false;
					 ...
					 //$('#tempp').html(response);
					 $('.confirm-email').html('<font color="green">'+conf_mail_sent+'</font>');
...
function ajax_add_cart(){
	...
	if(variation_count==2){
		...
		if($('#variation_one').val()==""){
			$('#Err_variation_one').html( lg_choose_color);
			$('#variation_one').focus();
		...
		if(AttrCountVal == 1){
			...
			//attrval2=attrVal1.split()
		}else if(AttrCountVal == 2){
			attrVal = $('#variation_one_name').val()+':'+$('#variation_one_val').val()+','+$('#variation_two_name').val()+':'+$('#variation_two').val();	
		}else{
			attrVal ='';
		}
...
var discount=$('#price').val()*$('#discountprice').val();
	...
	var actual_price=$('#price').val();
		...
		var price=parseFloat(actual_price)-parseFloat(discount);
			
		$.ajax({
			type: 'POST',
			url: baseURL+'site/cart/usercartadd',
			data: {'mqty':mqty,'quantity':quantity, 'product_id':product_id, 'sell_id':sell_id, 'price':price, 'attribute_values':attrVal,'digital_files':digital_files,'order_type':is_auctioned,'auction_id':auction_id,'pickup_option':pickup},
			success: function(response){
				//alert(response);
				//return false;
				var arr = response.split('|');
...
function ajax_add_gift_card(){
	...
	if(price ==''){
		$('#GiftErr').html(lg_pls_select_price_value);
		return false;		
	}
	if(rec_name ==''){
		$('#GiftErr').html(lg_enter_rxr_name);
		return false;		
	}
	if(rec_mail ==''){
		$('#GiftErr').html(lg_pls_enter_rxr_email);		
		return false;		
	}else{
	...
	if(descp =='' ){
		$('#GiftErr').html(lg_pls_enter_desc);		
		return false;
	}
	...
	var taxamount = $('#user_cart_tax_amount_'+selId).val();
		...
		if(code != '') {
			...
			success: function(response){
				...
				if(response == 1) {
					$('#CouponErr_'+selId).html(lg_Entered_code_is_invalid);
					return false;
				} else if(response == 2) {
...
function makeReview(usrId,prodId,dealCode){
	$.ajax({
			...
			success: function(response){
				...
				$.colorbox({width:"700px", height:"auto", open:true, inline:true, href:"#inline_reg1"});
				//window.location.reload();
			}
		});
}
	...
	$.ajax({
		...
		success: function(json){
			...
			if (json.status_code == 2){ 
				$('#alert_ownshopfav').trigger('click');
			}  else if(json.status_code == 1) {
			   //window.location.href = window.location.pathname;
			   
				if ( $('#fav').hasClass('fav-icon-sel') || $('#cart_fav').hasClass('fav1-btn') ){
				   $('#fav').removeClass('fav-icon-sel');
				   $('#fav').addClass('fa fa-heart');
				   $('#cart_fav').removeClass('fav1-btn')
				   $('#cart_fav').addClass('fav1-btn2')
				   
				}else{
				   $('#fav').removeClass('fa fa-heart');
				   $('#fav').addClass('fav-icon-sel');
				   $('#cart_fav').removeClass('fav1-btn2')
				   $('#cart_fav').addClass('fav1-btn')
				}
...
function changeProductToFavourite(pid,type,evt){		
//alert(evt);
if($(evt).hasClass('processing')) return false;
...
$(evt).css({'opacity':'0.4','cursor':'progress'});
	$.ajax({
		type:'post',
		...
		complete: function(){
			$(evt).css({'opacity':'1','cursor':'pointer'});
			$(evt).removeClass('processing');
		}
	});
}
...
$('#variation_one').change(function(e) {
var split_val = $('#variation_one').val();
//alert(split_val);
var variation1 = split_val.split('[');


$('#variation_one_val').val($.trim(variation1[0]));
   });
...
function shipping_validation()
{
	...
	{
	document.getElementById("err_city").innerHTML=cant_blank;
		err=1;
	}
	...
	{
	document.getElementById("err_postal").innerHTML=cant_blank;
		err=1;
	}else if( /[^a-zA-Z0-9\/]/.test( document.getElementById("postal").value ))
	{
		document.getElementById("err_postal").innerHTML="Postal code not alpha numeric";
		err=1;
	}
	...
	document.getElementById("err_phone").innerHTML='only number';
		err=1;
	}else if(phone.length > 15 || phone.length < 8 ){
		document.getElementById("err_phone").innerHTML='Invalid Phone number';
		err=1;
	}
	//alert( /[a-zA-Z0-9]/.test( document.getElementById("postal").value ));
	//alert(phone.length);
	if(err==1)
	{
	return false;	
...
function delete_user_list(){
	...
	if (!window.confirm(lg_sure)) return false;
...
function add_gift_card1(){
	...
	if(price ==''){
		$('#priceErr1').html(lg_Please_select_one);
	...
	if(rec_name ==''){
		$('.recipient_name1Err').html(lg_Please_enter_the_recipientname);
		$('.to').addClass('error');
		err=1;		
	}else{
        ...
        $.ajax({
			...
			$("#loadedImgshop").css("display", "none");
			  if(data=='Success'){
				  ...
				  $('#ErrImage').html(lg_success);
				  return true;
			  } else {
				  $('#ErrImage').css('color','#F00');
				  $('#ErrImage').html(lg_img_small);
				  return false;
			  }
		   },
            data: formData,
...
function giftcardprocess(){
	...
	if(giftvalue == false){
		$('#GiftCartErr').html('<span class="error">'+lg_payment_gateway+'</span>');
		//alert(lg_payment_gateway);
		return false;
	}else{
		$("#giftSubmit").submit();
	}	
...
function ticket_validation(){
	...
	var err=0;
	if(sub=='' || sub.length < 5){
		$('#errorSub').html(lg_pls_enter_sub_5char);
		$('#errorSub').css('color','red');
		$('#errorSub').css('float','initial');
		err=1;
	}
	if(desc=='' | desc.length < 5){
		$('#errorDesc').html(lg_pls_enter_des_5char);
		$('#errorDesc').css('color','red');
		$('#errorDesc').css('float','initial');
		err=1;
	}
	if(isNaN(prio)==true){
		$('#errorPri').html(lg_pld_selct_priority);
