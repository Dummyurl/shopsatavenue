...
class Adaptive_payments extends MY_Controller 
{
	...
	$Query = 'select sc.*,s.id as sid,s.owner_id,s.commission as scom, s.store_email,u.id as uid,u.commision as ucom,
			u.email as uemail from '.SHOPPING_CART.' as sc left join '.USERS.' as u 
			on u.id = sc.sell_id left join '.STORES.' as s on s.id = sc.store_id where sc.user_id = '.$this->checkLogin('U').'';
			$get_details = $this->checkout_model->ExecuteQuery($Query);
			$Details = array();
			$adminCommision = 0;
			if($get_details->num_rows() >0){
				foreach($get_details->result() as $_Details){
					$admin_commision = ($_Details->total * ($_Details->ucom/100));
					$adminCommision += $admin_commision;
					$scom = ($_Details->total * ($_Details->scom/100));
					$ucom = ($_Details->total - $scom - $admin_commision);
					if($_Details->sid!=1){
						if(isset($Details[$_Details->owner_id]['amount'])){
							$Details[$_Details->owner_id]['amount'] += $scom;
						}else{
							$Details[$_Details->owner_id]['amount'] = $scom;
							$Details[$_Details->owner_id]['email'] = $_Details->store_email;
						}
					}
		...
		$Receiver = array(
						...
						'Email' => 'vinu_business_4@gmail.com', 												// Receiver's email address. 127 char max.
						'InvoiceID' => '', 											// The invoice number for the payment.  127 char max.
		...
		{
			$errors = array('Errors'=>$PayPalResult['Errors']);
			//redirect('order/failure/'.$PayPalResult['Errors']);
			echo "<pre>"; print_r($errors); die;
		}
		else
		{
			// Successful call.  Load view or whatever you need to do here.
			redirect('order/success/'.$loginUserId.'/'.$lastFeatureInsertId.'/'.$PayPalResult['PayKey']);
