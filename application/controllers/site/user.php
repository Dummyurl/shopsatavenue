	...
	*/
	function fb_oauth() {
		...
		$facebook = new Facebook(array(
			'appId'		=>  '331478353931137',
			'secret'	=> 'bea15b8e63783e6d3f2b1d2c1daf0269',
			'xfbml'      => true,
			'cookie'      => true,
			));
			//get the user facebook id		
			$user = $facebook->getUser();
			$verify = count($_SESSION); 
			...
			if(empty($user)){
			  ...
			  $loginurl = $facebook->getLoginUrl(array(
							'scope'			=> 'publish_actions,publish_stream,read_stream,email, user_birthday, user_location, user_work_history, user_hometown, user_photos',
							'redirect_uri'	=> base_url().'site/user/fb_oauth_callback',
							'display'=>'popup'
							));
	...
	function fb_oauth_callback() {
		$_SESSION['facebook'] = "Verify";

	}
	
	function google_oauth() {

		// this is for error reporting if any 
		ini_set('display_errors', 1);
		ini_set('display_startup_errors', 1);
		error_reporting(E_ALL);			

		// you get autoload in vendor folder created by running composer installer as mentioned above
		require_once APPPATH .'libraries/google-api-php-client/vendor/autoload.php';
		require_once APPPATH .'libraries/google-api-php-client/src/Google/Client.php';
		require_once APPPATH .'libraries/google-api-php-client/src/Google/Service/Oauth2.php'; 

		$client = new Google_Client();
		$client->setAuthConfigFile( APPPATH . "libraries/client_secrets.json" );  //developer console credentials
		//$client->setRedirectUri( base_url(). 'oauth2callback.php' );
		$client->setRedirectUri( base_url(). 'site/user/google_oauth_callback' );
		$client->setScopes(array('https://www.googleapis.com/auth/userinfo.email','https://www.googleapis.com/auth/userinfo.profile'));
		
		if (isset($_SESSION['access_token']) && $_SESSION['access_token']) { 
		
			$client->setAccessToken($_SESSION['access_token']); //if there is access token already in the session
			$plus = new Google_Service_Oauth2($client); // get user info
		    $userinfo = $plus->userinfo->get();

			$user_details = $this->user_model->google_user_login_details( $userinfo['email'] );
			if ( count($user_details) > 0 ) {
				$userdata = array(
							'shopsy_session_user_id' => $user_details['id'],
							'shopsy_session_user_name' => $user_details['user_name'], 
							'shopsy_session_user_email' => $user_details['email'], 
							'shopsy_session_full_name' => $user_details['full_name'],
							'shopsy_session_user_confirm' => $user_details['is_verified'],
							'userType'=>$user_details['group']
						);
		  ...
		  // this is the callback url to which google will sent the access token containing require information you want by getting a google credential access for your application
		  $redirect_uri = base_url(). 'site/user/google_oauth_callback';
	...
	function google_oauth_callback() {
		...
		if (! isset($_GET['code'])) {
		  ...
		  //$redirect_uri = base_url('site/user/google_oauth_callback') .filter_var($auth_url, FILTER_SANITIZE_URL);
	...
	public function registerUser(){
		...
		$username = stripslashes(trim($this->input->post('email')));
			...
			if ( valid_email($email) ){
				...
				if ($duplicateName->num_rows()>0){
					...
					if ($duplicateMail->num_rows()>0){
						...
						$dataArr = array('full_name'=>$fullname,'user_name'=>$username,'last_name'=>$lastname,'email'=>$email,'password'=>$pwd,'status'=>'Active','is_verified'=>'No','affiliateId'=> $aff,'update_email'=>'Yes','notification_email'=>'follow,msg,like,lik_of_like,fav_shop');

						//$this->user_model->simple_insert('shopsy_users',$dataArr);
						//$returnStr['msg'] =  print_r( $dataArr, 1);
						//echo json_encode($returnStr);
						//exit(0);
						$this->db->insert('shopsy_users',$dataArr);
						$register_id = $this->db->insert_id();

						//$this->user_model->increaseUserCredits($register_id,$username,$email);
						$this->db->insert('sa_member_credit', array( 'user_id' => $register_id, 
																 'credit_amount' => 50.00, 
						...
						$checkUser = $this->user_model->get_all_details('shopsy_users',array('email'=>$email));
		...
		$acc_registered ="Your account Registerd with ";
	...
	public function signup_form(){
		...
		if ($this->checkLogin('U') != ''){
			...
			$this->load->view('site/user/signin.php',$this->data);
		}
	}
	...
	//die;
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			$stay_signed_in = (int) $this->input->post('stay_signed_in');
			...
			if ($checkUser->num_rows() == 1){
				...
				if ($checkUser->num_rows() == 1){
					...
					if ( $checkUser->row()->group == 'Seller' ) {
						$userdata['shop_name'] = $this->db->select('seller_businessname')->from('shopsy_seller')
													  ->where( array('seller_id' => $checkUser->row()->id ) )
													  ->get()->first_row()->seller_businessname;
					}
					$this->session->set_userdata($userdata);
					...
					if($stay_signed_in == 1 )
					{
						$CookieVal = array( 'name'   => 'saa_user','value'  => $this->session->userdata('shopsy_session_user_id'),'expire' => 3600*24*7);
				   ...
				   $this->user_model->updateShoppingCartItemsCount( $checkUser->row()->id );
				...
				$invalid_login=addslashes('Invalid login details' );
	...
	function myLikes() {
		...
		$this->data['page_url'] = "site/user/myLikes";
		...
		$this->data['product_list'] =  $this->db->select('p.id, p.product_name, p.seourl, p.image, p.user_id, p.store_id, p.price')
												->from('shopsy_product p')
												->join('saa_favorite_items f', 'f.p_id = p.id')
												->where( array('p.status' => 'Publish','f.user_id' => $user_id, 'f.favorite' => 1 ) )
		...
		$this->load->view('site/user/my_likes', $this->data);
	}
	...
	public function login_after_signup($userDetails=''){
		...
		if ($userDetails->num_rows() == 1){
			...
			$this->session->set_userdata($userdata);	
			$CookieVal = array( 'name'   => 'saa_user','value'  =>$userDetails->row()->id,'expire' => time()+3600*24*365,'secure' => FALSE);
			$this->input->set_cookie($CookieVal); 
	...
	public function logout_user(){
		//echo $this->input->cookie('saa_user');die;
		$datestring = "%Y-%m-%d %h:%i:%s";
		$time = time();
		$newdata = array(
		...
		if ($this->form_validation->run() === FALSE)
		{
			...
			if (valid_email($email)){
				...
				if ($checkUser->num_rows() == '1'){
					...
					$new_password=addslashes('New password sent to your mail');
					$this->setErrorMessage('success',$new_password);
					//redirect('signup');
					redirect('login');
				}else {
				$not_match=addslashes('Your email id not matched in our records');
					$this->setErrorMessage('error',$not_match);
					redirect('forgot-password');
				}
			}else {
			$not_valid=addslashes('Email id not valid');
	...
	public function send_user_password($pwd='',$query,$code,$id){
		...
		$subject = 'From: '.$this->config->item('email_title').' - '.$template_values['news_subject'];
  		$message .= '<!DOCTYPE HTML>
			...
			<title>'.$template_values['news_subject'].'</title>
			<body>';
		$message .= '<br><br><a href="'. $pwdlnk . '" title="reset password">Click this link to reset password!</a>';
		$message .= '</body>
			</html>';
...
//  		print_r($this->data['affiliates']); die;
	...
	function purchase_review( $pType='' )
	{
		...
		$this->data['orders'] = $this->db->select('o.*,os.order_status_name')->from('sa_order o')
										 ->join('sa_order_status os', 'os.order_status_id = o.order_status_id', 'left')
										 ->where( array('o.customer_id' => $user_id ) )
										 ->where( '( o.order_status_id = 1 OR o.order_status_id = 0 )' )
										 ->order_by('o.date_added desc')
										 ->get()->result_array();
										 
		$this->data['carriers'] = $this->db->select('carrier_code, carrier_name')->from('sa_carriers')->where('status = 1')->get()->result();
...
//			header('Content-type: image/jpeg');
   ...
   if($this->lang->line('user_cancel') != '') { $user_cancel= stripslashes($this->lang->line('user_cancel')); } else $user_cancel= "Cancel"; 
	...
	public function addMyLikes() {
		$json = array();
		$product_id = base64_decode( $this->input->post('data_id') );
		$userid = $this->checkLogin('U');

		$status = $this->user_model->addFavStatus( $userid, $product_id );
		if ( $status ) {
			$json = array( 'status' => 'success', 'message' => 'Item added to Favourite!' );
		} else {
			$json = array( 'status' => 'error', 'message' => 'Item already added to Favourite OR reached maximum limit' );
		}
		echo json_encode( $json );
	}
	
	public function removeMyLikes() {

		$json = array();
		$userid = $this->checkLogin('U');
		$pid = (int) $this->input->post('id');

		$status = $this->user_model->removeFavStatus( $userid, $pid );

		if ( $status ) {
			$json = array( 'status' => 'success', "message" => 'Successfully removed from list' );

		} else {
			$json = array( 'error' => 'error', "message" => 'Product is not found in list' );
		}
	...
	public function ajaxUserLogin() {
			...
			if ($this->form_validation->run() === FALSE)
			{
				$json = array( 'status' => 'error', 'message' => 'Email and password fields are required' );
				echo json_encode( $json );
				return;
			} 
			...
			if ($checkUser->num_rows() == 1){
					...
					if ($checkUser->num_rows() == 1){
					   ...
					   if($this->checkLogin('U')!=''){
						...
						$html = $this->load->view('site/templates/menu_after_login', '', true);
						$json = array( 'status' => 'success', 'message' => 'You are Logged In!', 'menu' => $html );
						echo json_encode( $json );
						return;
						  
					} 
					else{		
							$json = array( 'status' => 'error', 'message' => 'Your Account Is In-Active' );
							echo json_encode( $json );
							return;
					}
				
			} else {
				$json = array( 'status' => 'error', 'message' => 'Invalid login details' );
				echo json_encode( $json );
				return;
			}
