...
//	    	$this->data['purchasesList'] = $this->user_model->get_group_gifts_list($this->checkLogin('U'));
    ...
    public function save_manage_notification(){
    	$json = array();
    	if ($this->checkLogin('U') == '') {
    		redirect( base_url() . 'login' );
    	} else {

    		$dataArr['notification_email'] = $this->input->post('notification');
    		$dataArr['update_email'] = $this->input->post('updates');
	...
	function user_email_setting() {
		$this->data['page_url'] = "user-email-settings";

		$arr['user_id'] = $this->checkLogin('U');

		$this->data['page_items'] =  $this->user_model->get_email_settings($arr);

    	if ($this->checkLogin('U')==''){
    		redirect(base_url().'login');
		}
		$this->load->view('site/user/user_email_setting',$this->data);		
	}

	function save_user_email_setting() {

		if($this->input->post('forwhich')=="first"){

			$arr['newsale_discount'] = $this->input->post('newsale_discount');
			$arr['promo_emails'] = $this->input->post('promo_emails');
			$arr['user_id'] = $this->checkLogin('U');
			$returnArray = $this->user_model->update_email_settings($arr);

			echo json_encode($returnArray);
		}

		if($this->input->post('forwhich')=="second"){

			$arr['emailmeless'] = $this->input->post('sendless');
			$arr['user_id'] = $this->checkLogin('U');
			$returnArray = $this->user_model->update_email_settings($arr);
		...
		if($this->input->post('forwhich')=="third"){

			$arr['keepcoming'] = $this->input->post('keepcoming');
			$arr['user_id'] = $this->checkLogin('U');
    ...
    public function contact_support(){
    	$this->data['page_url'] = "contact-support";
    	if ($this->checkLogin('U')==''){
    		redirect(base_url().'login');
    	}else {
    		$this->data['heading'] = 'Contact Support';
			$condition = array('user_id'=>$this->checkLogin('U'));
	    	$this->load->view('site/user/contact_support',$this->data);
    	}
    }

    public function save_contact_support() {
		...
		$dataArr['query_type'] = $this->input->post('query_type');
		$dataArr['user_message'] = $this->input->post('user_message');
		$dataArr['user_id'] = $this->checkLogin('U');
		
		$this->user_model->simple_insert("saa_customer_queries", $dataArr );
		$json = array ( 'status' => "success", 'message' => 'Saved successfully!' );
    	echo json_encode($json);
    }
	function my_offers() {
		$this->data['page_url'] = "my-offers";

		$arr['user_id'] = $this->checkLogin('U');


		$this->data['page_items'] =  $this->user_model->get_my_offers($arr);

    	if ($this->checkLogin('U')==''){
    		redirect(base_url().'login');
		}
		$this->load->view('site/user/my_offers',$this->data);		
	}
