...
// 			</tr>';
	 ...
	 *
	 */
	public function closedclaim() {		#echo '<pre>'; print_r($_POST); die;
		$orderid=$this->input->post('orderidd');
...
/**************** Zendesk  Create Ticket For Dispute Open End  **************/
	...
	public function getOrder() {
		...
		$user_id = $this->checkLogin('U');
		$order = $this->db->select('*')->from('sa_order')->where( array( 'order_id' => $order_id, 'customer_id' => $user_id ) )
						  ->get()->first_row();
		if( $order->customer_id != $user_id ) {
			$json = array(
				 'status' => 'error',
				 'message' => 'You are not authorized to veiw this order!'
			);
			exit(0);
		}
		
		$products = $this->db->select('op.*, p.image, s.seller_businessname as shop_name')->from('sa_order_product op')
							 ->join('shopsy_product p', 'p.id = op.product_id', 'left' )
							 ->join('shopsy_seller s', 'op.store_id = s.id' , 'left' )
							 ->where( array('op.order_id' => $order_id) )
							 ->order_by('op.store_id')->get()->result();
		$json = array(
		     'status' => 'success',
			 'order' => $order,
			 'products' => $products
	...
	public function getShippedOrder() {
		...
		$shipments = $this->db->select('os.carrier, os.ship_date, os.tracking_no, op.*, p.image, s.seller_businessname as shop_name')
							 ->from('sa_order_shipments os')
							 ->join('sa_order_product op', 'op.order_product_id = os.order_product_id', 'left' )
							 ->join('shopsy_product p', 'p.id = op.product_id', 'left' )
							 ->join('shopsy_seller s', 'op.store_id = s.id' , 'left' )
							 ->where( array('os.customer_id' => $user_id, 'delivered_date' => '' ) )
							 ->order_by('os.ship_date')->get()->result();
	...
	public function getCompletedOrder() {
		...
		$shipments = $this->db->select('os.carrier, os.ship_date, os.delivered_date, os.tracking_no, op.*, p.image, s.seller_businessname as shop_name, DATEDIFF( CURDATE() , os.delivered_date ) AS days')
							 ->from('sa_order_shipments os')
							 ->join('sa_order_product op', 'op.order_product_id = os.order_product_id', 'left' )
							 ->join('shopsy_product p', 'p.id = op.product_id', 'left' )
							 ->join('shopsy_seller s', 'op.store_id = s.id' , 'left' )
							 ->where( 'os.delivered_date is not null ' )
							 ->where( array('os.customer_id' => $user_id ) )
							 ->order_by('os.ship_date')
							 ->get()->result();
	...
	public function getReturnDetails() {
		...
		$user_id = $this->checkLogin('U');
		$order_product_id = $this->input->post('req_id');
		$rtn_product_qry = $this->db->select('op.*, p.image,, s.seller_businessname as shop_name')->from('sa_order_product op')
									->join('sa_order o', 'o.order_id = op.order_id', 'left' )
							 		->join('shopsy_product p', 'p.id = op.product_id', 'left' )
							 		->join('shopsy_seller s', 's.id = op.store_id' , 'left' )
									->where( array( 'o.customer_id' => $user_id, 'op.order_product_id' => $order_product_id ) )
									->get();
		if( ! $rtn_product_qry->num_rows() ) {
			$json = array(
				 'status' => 'error',
			...
			$rtn_product = $rtn_product_qry->first_row('array');
		}
		
		//$allowed_qty = 0;
		$shipment_qry = $this->db->select('*')->from('sa_order_shipments')->where(array('customer_id' => $user_id, 'order_product_id' => $order_product_id))
								 ->get();

		if( $shipment_qry->num_rows() ) {
			$shipment = $shipment_qry->first_row('array');
			//if( $shipment['quantity'] < $rtn_product['quantity'] ) $allowed_qty = $rtn_product['quantity'] - $shipment['quantity'];
			//else $allowed_qty = 0;
		} else {
			$json = array(
				 'status' => 'error',
				 'message' => 'Product is not found in shipment!'
			);
			echo json_encode( $json );
			exit(0);
		}
		...
		$json = array(
			 ...
			 'rtn_last_date' => date( 'm/d/Y', strtotime( "+14 day" ) )
		);

		echo json_encode( $json );
	}
	
	public function saveProductReturn() {
		$json = array();
		...
		$rtn_reason = $this->input->post('rtn_reason');
		$order_product_qry = $this->db->select('op.*')->from('sa_order_product op')
									  ->join('sa_order o', 'o.order_id = op.order_id', 'left')
									  ->where( array('o.customer_id' => $user_id, 'op.order_product_id' => $order_product_id) )
									  ->get();
		if( ! $order_product_qry->num_rows() ) {
			 echo json_encode( array( 'status' => 'error', 'message' => 'Product is not found in Your Order!' ) );
			 exit(0);
		}
		$order_product = $order_product_qry->first_row();
		$data = array(
				'customer_id' => $user_id,
				'order_id' => $order_product->order_id,
				'store_id' => $order_product->store_id,
				'order_number' => $order_product->store_order_id,
				'order_product_id' => $order_product->order_product_id,
				'quantity' => $rtn_qty,
				'return_reason' => $rtn_reason,
				//'return_carrier' => '',
				//'return_tracking_no' => '',
		...
		$this->db->insert('sa_order_return', $data);
		$rma_id = $this->db->insert_id();
		if ( (int) $rma_id == 0 )  {
			 echo json_encode( array( 'status' => 'error', 'message' => "Can't creat return request. please contact customer support!" ) );
			 exit(0);
		}
		$ship_address = $this->db->select('legal_name, ship_address1, ship_address2, ship_city, ship_state, ship_zip')
							 ->from('sa_shop_bizinfo')
							 ->where( array('shop_id' => $order_product->store_id) )
							 ->get()->first_row( 'array' );

		$this->data['rma_id'] = $rma_id;
		$this->data['ship_address'] = $ship_address;
		$message = $this->load->view('site/user/return_success_message' , $this->data, true);
		$json = array (
					'status' => 'success',
					'message' => $message,
				);
	...
	public function getOrderReturns() {
		...
		if ( $this->data['loginCheck'] == '' ) {
			echo json_encode( array( 'status' => 'error', 'message' => 'You need to login!' ) );
			exit(0);
		}
		$user_id = $this->checkLogin('U');
		$returns_qry = $this->db->select('r.rma_id, r.order_id, r.quantity, r.date_created as rtn_date, r.return_status, r.return_carrier,  r.return_tracking_no, op.product_id, op.name, p.image, s.return_status_name')
								->from('sa_order_return r')
								->join('sa_return_status s', 's.return_status_id = r.return_status', 'left' )
								->join('sa_order_product op', 'op.order_product_id = r.order_product_id', 'left' )
								->join('shopsy_product p', 'p.id = op.product_id' , 'left' )
								->where( array('customer_id' => $user_id) )
								->order_by('r.rma_id desc')
								->get();
		if( $returns_qry->num_rows() ) {
			$returns = $returns_qry->result();
		} else {
			$returns = array();
		}
	...
	public function addReturnCarrier() {
		$json = array();
		...
		$user_id = $this->checkLogin('U');
		$carrier = $this->db->select('carrier_name')->from('sa_carriers')
						->where( array('carrier_code' => $this->input->post('carrier') ) )
						->get()->first_row();
		$data = array(
			'return_carrier' => $carrier->carrier_name,
			'return_tracking_no' => $this->input->post('tracking_no')
		);
		$this->db->update('sa_order_return', $data, array('rma_id' => $this->input->post('rtn_rma_id'), 'customer_id' => $user_id) );
