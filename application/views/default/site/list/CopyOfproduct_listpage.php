				...
				<a class="<?php if($activeCatid[0] == $catList->id) { echo 'active';}?>" href="<?php if($activeCatid[0] == $catList->id) { 
					echo 'javascript: void(0);'; 
							...
							if($super_sub_catStatus=='No' && $super_sub_catID == $catList->id){ 
								...
								echo $lnk1.$catList->id.'-'.$catList->seourl; 
							} 
						} else { 
      ...
      <div class="col-md-9 pull-right filter-button-right col-xs-12 ">
        <div class="filter-button-inside"> 
		 ...
		 <?php if($this->input->get('color') != '') {?>
			...
			<div  id="filter_byColor" class="filter-button" style="display:none;"><a href="javascript:void(0);">Color</a></div>	
			<?php } else {?>	
		   <div  id="filter_byColor" class="filter-button"><a href="javascript:void(0);">Color</a></div>
        <?php }?>
				 ...
				 <div class="collections-ui">
					 ...
					 <div  class="favorite-container">
					  <?php if($loginCheck !=''){
						...
						if(empty($favArr)){ ?>
							 <button onclick="return changeProductToFavourite('<?php echo stripslashes($products['id']); ?>','Fresh');" data-source="casanova"  class="btn-fave  inline-overlay-trigger btn-fave-action" type="button"> 
								<span class="icon"></span> <span class="ie-fix">&nbsp;</span> 
							 </button>
						<?php } else {?>
							<button onclick="return changeProductToFavourite('<?php echo stripslashes($products['id']); ?>','Old');" data-source="casanova"  class="btn-fave  inline-overlay-trigger btn-fave-action" type="button"> 
...
$(document).ready(function(){
  $("#handmade").click(function(){    
	  ...
	  if (history.pushState) {
	     window.history.pushState("object or string", "Title",fullUrl+"?marketplace="+this.id+shipfrom+shipto+pricefilter+giftcard);
	  }
	 ...
	 if(filterPrice==2)
	  ...
	  if(minPrice != '' || maxPrice != '')
	  {
		  ...
		  if(filterPrice!=''){
			  ...
			  if (history.pushState) {
			    window.history.pushState("object or string", "Title",fullUrl+'?'+minp+maxp+shipto+giftcard+madeby+shipfrom);
			  }
			  window.location.reload();
		      ...
		      if (history.pushState) {
		      window.history.pushState("object or string", "Title",fullUrl+'?'+giftcard+madeby+shipto+shipfrom);
			  }
			  window.location.reload();
...
$(document).click(function(){ 
  $("#filterByship_to").change(function(){ 
		  ...
		  if(filtercountry != ''){  
			  ...
			  if (history.pushState) {
			   window.history.pushState("object or string", "Title",fullUrl+'?shipto='+filtercountry+giftcard+madeby+shipfrom+pricefilter);
			  }
			  window.location.reload();
			  ...
			  $("#tiles").html(data); 
		       ...
		       if (history.pushState) { 
		       window.history.pushState("object or string", "Title",fullUrl+'?'+giftcard+madeby+shipfrom+pricefilter);
			   }
			   window.location.reload();
...
function ship_fromFilter(){
		...
		$('#fromloc_clicked').html('<div  class="filter-group selected-state"><div id="restore_from" style="display: inline-block;"><div class="filter-input facet"><label><?php if($this->lang->line('list_items') != '') { echo stripslashes($this->lang->line('list_items')); } else echo 'Items from'; ?> </label><input type="hidden" class="filtering-input" value="'+filtercountry+'" name="filter_byshipFrom" id="filter_byshipFrom"/> '+filtercountry+'</div><input class="filter-remove filter-submit" type="button" id="remove_from" onclick="return remove_fromFilter();" value="x"></div></div>');
		  ...
		  if(filtercountry != ''){
			  ...
			  if (history.pushState) {
			   window.history.pushState("object or string", "Title",fullUrl+'?location='+filtercountry+shipto+giftcard+madeby+pricefilter);
			  }
			  window.location.reload();
			 /*  $.get('site/product/Load_ajax_products_list?cat_id='+Catid+'&location='+filtercountry+giftcard+shipto+madeby+pricefilter, function(data) {
		      ...
		      if (history.pushState) {
		        window.history.pushState("object or string", "Title",fullUrl+'?'+filtercountry+giftcard+shipto+madeby+pricefilter);
			  }
			  window.location.reload();
...
$(document).ready(function(){ 
  $("#giftcard").change(function(){
		  ...
		  var fullUrl=$('#fullurl').val();
		  if (history.pushState) {
		    window.history.pushState("object or string", "Title",fullUrl+'?'+giftcard+shipto+shipfrom+madeby+pricefilter);
		  }
		  window.location.reload();
	 ...
	 if(filterPrice==2)
	  ...
	  if(minPrice != '' || maxPrice != '')
	  {
		  ...
		  if(filterPrice!=''){
		      ...
		      if (history.pushState) {
		      window.history.pushState("object or string", "Title",fullUrl+'?'+giftcard+shipto+madeby+shipfrom);
			  }
			  window.location.reload();
