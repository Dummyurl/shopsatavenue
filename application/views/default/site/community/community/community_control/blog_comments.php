...
<section class="container">
        ...
        <ul class="bread_crumbs">
            ...
            <span>&rsaquo;</span>
            <li> <?php if($this->lang->line('com_comments') != '') { echo stripslashes($this->lang->line('com_comments')); } else echo "Comments"; ?></li>
        </ul>
     <div class="clear"></div>
     <div class="hole_content">
      <div class="heading"><?php if($this->lang->line('com_comments') != '') { echo stripslashes($this->lang->line('com_comments')); } else echo "Comments"; ?></div>
        	<?php  $this->load->view('site/community/community/community_control/blog_leftside.php'); ?>
            <form name="seekerActionForm" id="seekerActionForm" method="post" enctype="multipart/form-data" action="site/community/commentInactiveDeleteFunction" >	
  ...
  <td><input type="checkbox" name ="seekerCheckbox[]" id="seekerCheckbox"  onclick="return checkBoxController(this.form,'seekerCheckbox[]',this.checked);"/></td>
   <td width="20%" ><strong><?php if($this->lang->line('com_commenter') != '') { echo stripslashes($this->lang->line('com_commenter')); } else echo "Commenter"; ?></strong></td>
   <td width="20%" ><strong><?php if($this->lang->line('com_posttitle') != '') { echo stripslashes($this->lang->line('com_posttitle')); } else echo "Post Title"; ?></strong></td>
   <td width="20%" ><strong><?php if($this->lang->line('com_comments') != '') { echo stripslashes($this->lang->line('com_comments')); } else echo "Comments"; ?></strong></td>
   ...
   <td width="35%" ><strong><?php if($this->lang->line('com_rsponseto') != '') { echo stripslashes($this->lang->line('com_rsponseto')); } else echo "In Response To"; ?></strong></td>
 </tr>
 <thead>
 <tbody>
 <?php #echo '<pre>'; print_r($commentData);  
 foreach($commentData as $details){?>
  <?php if($details['comment_status']!= 'Unpublish') { ?>
 <tr>
  <td><input type="checkbox" class="caseSeeker" name="seekerCheckbox[]" value="<?php echo $details['comment_id']; ?>" /></td>
   <td><a href="view-people/<?php echo $details['user_name'];?>"><?php echo stripslashes($details['user_name']);?></a></td>
   <td><a href="<?php echo $details['post_id'];?>/news-details"><?php echo stripslashes($details['post_title']);?></a></td>
   <td><?php echo character_limiter(stripslashes($details['comment_body']), 30);?></td>
   <td><?php echo stripslashes($details['comment_date']);?></td>
   <td><div class="edit">
   <!--<a href="#" style="padding-left:0px;"><img src="images/reply.png"  /></a>-->
   <a href="site/community/delete_comments/<?php echo stripslashes($details['comment_id']);?>" onclick="return changeStatusCommon('delete');"><img src="images/delete.png"  /></a>
   <?php if($details['comment_status'] == 'active') { ?>
        <a  href="site/community/update_comments/<?php echo stripslashes($details['comment_id']);?>/<?php echo stripslashes($details['comment_status']);?>" onclick="return changeStatusCommon('<?php echo $details['comment_status']; ?>');"><img src="images/active.png"  /></a>
   <?php } else { ?>
		 <a  href="site/community/update_comments/<?php echo stripslashes($details['comment_id']);?>" onclick="return changeStatusCommon('<?php echo $details['comment_status']; ?>');"><img src="images/inactive.png"  /></a>
    <?php } ?>
   </div>
   </td>
 </tr>
