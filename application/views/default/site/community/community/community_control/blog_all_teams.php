...
<section class="container">
        ...
        <ul class="bread_crumbs">
            ...
            <li> <a href="settings/my-account/<?php echo $this->session->userdata['shopsy_session_user_name']; ?>" class="a_links"><?php if($this->lang->line('comm_account') != '') { echo stripslashes($this->lang->line('comm_account')); } else echo "Your Account"; ?></a></li>
             <span>&rsaquo;</span>
            <li> <?php if($this->lang->line('user_teams') != '') { echo stripslashes($this->lang->line('user_teams')); } else echo "Teams"; ?></li>
        </ul>
     <div class="clear"></div>
     <div class="hole_content">
      <div class="heading"><?php if($this->lang->line('user_team') != '') { echo stripslashes($this->lang->line('user_team')); } else echo "Team"; ?></div>
        	<?php  $this->load->view('site/community/community/community_control/blog_leftside.php'); ?>
             <form name="seekerActionForm" id="seekerActionForm" method="post" enctype="multipart/form-data" action="site/community/teamActiveInactiveDeleteFunction" >	
            ...
            <div class="right_split">
                    ...
                    <a class="btn btn-success see_more" style="float:left" href="add-team" ><i class="icon-ok"></i> <?php if($this->lang->line('com_addteam') != '') { echo stripslashes($this->lang->line('com_addteam')); } else echo "Add Team"; ?></a>
  ...
  <tr style="background:#f4f4f4;">
   ...
   <td width="20%" ><strong><?php if($this->lang->line('com_teamlogo') != '') { echo stripslashes($this->lang->line('com_teamlogo')); } else echo "Team Logo"; ?></strong></td>
  <!-- <td width="20%" ><strong>Comments Count</strong></td>-->
   <td width="20%" ><strong><?php if($this->lang->line('com_addeddate') != '') { echo stripslashes($this->lang->line('com_addeddate')); } else echo "Added Date"; ?></strong></td>
 ...
 foreach($postData as $details){?>
  <?php if($details['status']!= 'Unpublish') { ?>
 <tr>
 <td><input type="checkbox" class="caseSeeker" name="seekerCheckbox[]" value="<?php echo $details['id']; ?>" /></td>
   <td><a href="team/<?php echo $details['id']; ?>/<?php echo $details['teamSeourl']; ?>"><?php echo stripslashes($details['teamName']);?></td>
   <?php if($details['teamImage']!=''){ ?>
   <td><img width="50px;" src="<?php echo base_url().TEAM_PATH.$details['teamImage']; ?>" alt="<?php echo $details['teamName']; ?>" /></td>
   <?php }else{ ?>
   <td><img width="50px;" src="<?php echo base_url().TEAM_PATH.'no-team-logo.png'; ?>" alt="<?php echo $details['teamName']; ?>" /></td>
   <?php } ?>
  	
   <td><?php echo stripslashes($details['teamAddDate']);?></td>
   <td><div class="edit">
   <?php if($details['status'] == 'Active') { ?>
        <a  href="site/community/update_teams/<?php echo stripslashes($details['id']);?>/<?php echo stripslashes($details['status']);?>" onclick="return changeStatusCommon('<?php echo $details['status']; ?>');"><img src="images/active.png" title="<?php echo ucfirst($details['status']); ?>"  /></a>
   <?php } else { ?>
		 <a  href="site/community/update_teams/<?php echo stripslashes($details['id']);?>/<?php echo stripslashes($details['status']);?>" onclick="return changeStatusCommon('<?php echo $details['status']; ?>');"><img src="images/inactive.png" title="<?php echo ucfirst($details['status']); ?>"  /></a>
    <?php } ?>

    <a href="edit-team/<?php echo stripslashes($details['id']);?>" style="padding-left:0px;"><img src="images/edit.png" title="Edit"   /></a>
   <a href="site/community/delete_team_v/<?php echo stripslashes($details['id']);?>" onclick="return changeStatusCommon('delete');"><img src="images/delete.png" title="Delete"  /></a>
