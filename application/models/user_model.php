...
//echo "GV:" . $this->db->last_query(); exit(0);

		$this->session->set_userdata( 'cart_quantity', $count );
	}

	public function get_email_settings($inarray){

		$data = array();
		$data['user_id'] = $inarray['user_id'];
		
		$this->db->select('*');
		$this->db->from("saa_mail_settings");
	...
	public function update_email_settings($inarray){
		...
		if( $query->num_rows() > 0 )
		{
			...
			if(isset($inarray['newsale_discount'])){
				$data['newsale_discounts'] = $inarray['newsale_discount'];
			}
			
			if(isset($inarray['promo_emails'])){
			$data['promotional'] = $inarray['promo_emails'];

			}
			if(isset($inarray['emailmeless'])){
			$data['emailmeless'] = $inarray['emailmeless'];
			}

			if(isset($inarray['keepcoming'])){
			$data['keepcoming'] = $inarray['keepcoming'];
			}
	
			$this->db->where('user_id', $data['user_id']);
				$this->db->update("saa_mail_settings", $data); 
		...
		else
		{
			...
			// return $data;	
				$this->db->insert("saa_mail_settings", $data); 
					$data['message'] = "Successfully Saved";
		}
		return $data;
	
	}
	public function get_my_offers($inarray){

		$data = array();

		$userid = $inarray['user_id'];
		
		$this->db->select('saop.name,saop.price,saop.disc_amount,saop.shipping_paid,saop.quantity,saop.total,pd.image,pd.id as pid');
		$this->db->from('sa_order as sao');
		$this->db->join('sa_order_product as saop', 'sao.order_id = saop.order_id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = saop.product_id');		
		$this->db->where('sao.customer_id = "'.$userid.'"');
	...
	function addFavStatus( $user_id=0, $product_id=0 ) {
		$json = array();
		
		$total = $this->db->select('count(*) as total')->from('saa_favorite_items')->where( array( 'user_id' => $user_id, 'p_id' => $product_id ) )->get()->first_row()->total;
		if ( ! $total ) {
			$this->db->query( "UPDATE shopsy_product SET likes = likes + 1 WHERE id = '" . $product_id . "' " );
			$actArr = array(
			...
			$condition = array('activity_id' => $product_id, 'user_id' => $user_id );
			$this->db->update('saa_user_activity', $actArr, $condition );
			...
			$this->db->insert('saa_notifications', $actArr);
			
			$shop_id = $this->db->select('store_id')->from('shopsy_product')->where( array( 'id' => $product_id ) )->get()->first_row()->store_id;
			$this->db->query( "INSERT INTO saa_favorite_items SET shop_id = '" . $shop_id . "', user_id = '" . $user_id . "', p_id = '" . $product_id . "', favorite = '1' " );
			return true;
	...
	function removeFavStatus( $user_id = 0, $product_id = 0 ) {
		$total = $this->db->select('count(*) as total')->from('saa_favorite_items')->where( array( 'user_id' => $user_id, 'p_id' => $product_id ) )->get()->first_row()->total;
		if ( $total ) {
			$this->db->query( "UPDATE shopsy_product SET likes = likes + 1 WHERE id = '" . $product_id . "' " );
			$actArr = array(
			...
			$this->db->query( "DELETE FROM saa_favorite_items WHERE user_id = '" . $user_id . "' AND p_id = '" . $product_id . "' LIMIT 1 " );
			return true;
		}
	...
	function getUserCreditBalance( $user_id = 0 ) {
		$now = date('Y-m-d H:i:s');
		$credit_qry = $this->db->select('credit_amount')->from('sa_member_credit')->where( array( 'user_id' => $user_id ) )->where( "expiry_date > '" . $now . "'")->get();
		if( $credit_qry->num_rows == 0 ) return 0.0;
		
		$credit_amount = $credit_qry->first_row()->credit_amount;
