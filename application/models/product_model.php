...
class Product_model extends My_Model
{
	...
	***/
	...
	/***  Get Draft product images
	***/
	public function get_draft_product_images( $product_id = '') {
		...
		$this->db->select( 'product_name,image,video_url' );
	...
	/***  Add product image
	      param 1. image name ( images are delimited by char | )
	***/
	public function add_product_image( $dataArr='' ) {
		...
		$this->db->from( PRODUCT );
		$this->db->where('id', $dataArr['product_id'] );
		$this->db->where('user_id', $user_id );
		$this->db->where('status', 'Draft' );
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
			...
			$this->db->where('user_id', $user_id );
			$this->db->where('status', 'Draft' );
			$update_arr['modified'] = 'NOW()';
			$update_arr = array();
			if ( $result['image'] == '' ) {
				$update_arr['image'] = $dataArr['image_name'];
				$this->db->update(PRODUCT,$update_arr);
			} else {
				$images = explode("|",$result['image']);
				if( count($images) >= 5 ) {
					return 0;
				}
				$update_arr['image'] = $result['image'] . '|' . $dataArr['image_name'];
				$this->db->update(PRODUCT,$update_arr);
	...
	/***  Add product Video
	      param 1. Video url
	***/
	public function add_product_video( $dataArr='' ) {
		...
		if ( $query->num_rows() ) {
			...
			//		return 0;
			//} else {
				$update_arr['video_url'] = $dataArr['video_url'];
				$this->db->update(PRODUCT,$update_arr);
				return 1;
	...
	***/
	public function edit_product($dataArr='',$condition='',$table='') { 
			...
			$this->db->update( PRODUCT, $dataArr, $condition );
	}
	/*** To edit dispute 
		Param 	1.Array $dataArr to add
					2.Array $Condition to check
	...
	*****   getProductInfo 
	*****/
	public function getProductInfo( $product_id, $publish = false ) {

		$fields = "shopsy_product.id, product_name, category_id, product_sort, description, meta_keyword, shopify_url, sold_exclusive, maked_on, shopsy_category.cat_name, shopsy_product.meta_description, shopsy_product.image";
		$user = $this->session->all_userdata();
		if ( $this->checkLogin('A') != ''){
			$user_id = $user['shopsy_session_admin_id'];
		} elseif ($this->checkLogin('U') != '' ) {
		...
		$this->db->from( PRODUCT );
		$this->db->join('shopsy_category','shopsy_category.id = shopsy_product.category_id', 'left');
		$this->db->where('shopsy_product.id', $product_id );
		$this->db->where('user_id', $user_id );
		if( $publish ) {
			$this->db->where('shopsy_product.status', 'Publish' );
		} else {
			$this->db->where('shopsy_product.status', 'Draft' );
		}
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
			return $query->row_array();
    ...
    /****
	*****   get_draft_product_variations 
	*****/
	public function get_draft_product_variations( $product_id ) {

		$fields = "id, product_name, price, msrp, price, sku, quantity, upc, part_number,variable_product, variable_sku, variable_upc, variable_price, variable_msrp, variable_mpn, image";
		$user = $this->session->all_userdata();
		if ( $this->checkLogin('A') != ''){
			$user_id = $user['shopsy_session_admin_id'];
		} elseif ($this->checkLogin('U') != '' ) {
		...
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
			$variation = $query->row_array();
		}

		if ( $variation['variable_product'] ) {
			$this->db->select( '*' );
			...
			if ( $query->num_rows() ) {
				 foreach( $query->result_array() as $options ) {
					$variation['product_options'][] =  $options;
				 }
			}

			for($i=0; $i < count( $variation['product_options'] ) ; $i++ ) {
				$this->db->select( '*' );
				$this->db->from( 'shopsy_product_option_values' );
				$this->db->where('product_id', $product_id );
				$this->db->where('product_option_id', $variation['product_options'][$i]['product_option_id'] );
				...
				if( $query->num_rows() ) {
					 ...
					 $variation['product_options'][$i]['values'] = $option_values;
				}
			}
	...
	*****   get_draft_product_variations 
	*****/
	public function get_draft_product_variants( $product_id ) {
		...
		$variation = array();
		$this->db->select( '*' );
		$this->db->from( 'shopsy_product_variation' );
		$this->db->where('product_id', $product_id );
		$query = $this->db->get(); 
		$var_data = array();
		if ( $query->num_rows() ) {
			 foreach( $query->result_array() as $value_row ) {
				 $var_data[] = $value_row;
			 }
			return $var_data;
		}
    ...
    /****
	*****   get_draft_product_shipping
	*****/
	public function get_draft_product_shipping( $product_id ) {

		$fields = "id, product_name, ship_days, ship_price_info, return_policy, weight, ship_length, ship_width, ship_height";
		$user = $this->session->all_userdata();
		if ( $this->checkLogin('A') != ''){
			$user_id = $user['shopsy_session_admin_id'];
		} elseif ($this->checkLogin('U') != '' ) {
		...
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
			$shipping['info_fields'] = $query->row_array();
		}
		
		$this->db->select( "product_id, code, ship_price, next_item_price" );
		$this->db->from( 'shopsy_product_shipping' );
		$this->db->where('product_id', $product_id );
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
			foreach( $query->result() as $rates ) {
				$code = $rates->code;
				$shipping['rates'][$code] = array(
											'ship_price' => $rates->ship_price,
											'next_item_price' => $rates->next_item_price
											);
   ...
   ***/
	...
	***/
	public function view_product_details($condition = '',$opt=''){
		if($opt==''){
			$select_qry = "select p.user_id,p.id,p.category_id,p.seourl,p.quantity,p.product_name,p.price,p.image,p.base_price,p.status,p.created,p.product_promoted,p.action,p.discount,u.id as sellerid,u.email as selleremail,u.full_name,u.user_name,u.thumbnail,p.product_featured,p.pay_status,p.feature_expire,s.seller_businessname,fp.id as fp_id,fp.pack_id,fp.start_date,fp.page from ".PRODUCT." p LEFT JOIN ".USERS." u on p.user_id=u.id LEFT JOIN ".SUB_SHIPPING." ss on p.id=ss.product_id  LEFT JOIN ".SELLER." s on p.user_id=s.seller_id LEFT JOIN shopsy_subproducts sub on p.id= sub.product_id LEFT JOIN ".FEATURE_PRODUCT." fp on fp.product_seo = p.seourl".$condition;
		 }else{
			$select_qry = "select p.user_id,p.id,p.category_id,p.quantity,p.seourl,p.product_name,p.price,p.image,p.base_price,p.status,p.created,p.product_promoted,p.action,p.discount,u.id as sellerid,u.email as selleremail,u.full_name,u.user_name,u.thumbnail,p.product_featured,p.pay_status,p.feature_expire,s.seller_businessname,fp.id as fp_id,fp.pack_id,fp.start_date,fp.page from ".PRODUCT." p LEFT JOIN ".USERS." u on p.user_id=u.id   LEFT JOIN ".SUB_SHIPPING." ss on p.id=ss.product_id  LEFT JOIN ".SELLER." s on p.user_id=s.seller_id LEFT JOIN shopsy_subproducts sub on p.id= sub.product_id LEFT JOIN ".FEATURE_PRODUCT." fp on fp.product_seo = p.seourl ".$condition;
		}
	...
	public function view_product_details_feature($condition = '',$opt=''){
		if($opt==''){
			...
			$select_qry = "select p.user_id,p.id,p.category_id,p.quantity,p.seourl,p.product_name,p.price,p.image,p.base_price,p.status,p.created,p.product_promoted,p.action,p.discount,u.id as sellerid,u.email as selleremail,u.full_name,u.user_name,u.thumbnail,p.deal_date_to,p.deal_time_to,p.product_featured,p.pay_status,p.feature_expire,s.seller_businessname,fp.id as fp_id,fp.pack_id,fp.start_date,fp.page from ".PRODUCT." p LEFT JOIN ".USERS." u on p.user_id=u.id   LEFT JOIN ".SUB_SHIPPING." ss on p.id=ss.product_id  LEFT JOIN ".SELLER." s on p.user_id=s.seller_id LEFT JOIN shopsy_subproducts sub on p.id= sub.product_id JOIN ".FEATURE_PRODUCT." fp on fp.product_seo = p.seourl ".$condition;
		}
...
/*		public function getFeatureProduct($userId=''){
   ...
   // Param	1. integer $userId
	...
	public function get_recent_product_details() {
		...
		$this->db->select('p.*,p.seourl as product_seourl,u.thumbnail,u.user_name,s.seller_businessname,s.seourl as shopurl,s.review_count,s.shop_ratting, s.hide_review as seller_hide_review');
		$this->db->from(PRODUCT.' as p');
	...
	public function get_featured_product_details(){
		...
		$this->db->select('p.*,u.thumbnail,u.user_name,u.full_name,s.seourl as shop_seourls,s.review_count,s.shop_ratting, s.hide_review  as seller_hide_review');
		$this->db->from(PRODUCT.' as p');
		$this->db->join(USERS.' as u','p.user_id=u.id');
		$this->db->join(SELLER.' as s','p.user_id=s.seller_id');
	...
	public function get_category_link( $product_id = '' ) {

		$select_qry = "select category_id from ".PRODUCT. " where id = ". $product_id;
		$catList = $this->ExecuteQuery($select_qry);
		$cat_row =  $catList->row_array();
		$cat_link = '';
		if ( $cat_row['category_id']  != '' ) {
			$select_qry = "SELECT id, cat_name, seourl FROM shopsy_category WHERE id IN ( " . $cat_row['category_id'] . " ) AND status = 'Active' ";
			$result = $this->ExecuteQuery($select_qry);
			foreach( $result->result_array() as $row ) {
				if ( $cat_link != '' ) { $cat_link .= " / "; }
				$cat_link .= '<span><a href="search/all/' . $row['id'] . "-" . $row['seourl'] .'" >' . $row['cat_name'] . '</a></span>';
			}
		}
		return $cat_link;

	}
	public function get_cart_product_variations( $product_id = '') {
		/*$select_qry = "SELECT pv.*, ov.color_code, o.option_type_id,o.product_option_name FROM `shopsy_product_variation` pv
INNER JOIN shopsy_product_option_values ov ON ov.product_option_value_id = pv.product_variant_name
INNER JOIN shopsy_product_options o ON o.product_option_id = ov.product_option_id
WHERE pv.`product_id` = " . $product_id;*/
		$select_qry = "SELECT pv.* FROM `shopsy_product_variation` pv
WHERE pv.`product_id` = " . $product_id;

		$var_res = $this->ExecuteQuery($select_qry);
		
		return $var_res->result_array();

	}
	
	public function get_product_option_values( $product_id = 0 ) {
		$select_qry = "SELECT product_option_value_id, product_option_id, option_value FROM shopsy_product_option_values WHERE product_id = ";
		$select_qry .= $product_id . " ORDER BY option_seq_id ASC ";
		$var_res = $this->ExecuteQuery($select_qry);
		
		return $var_res->result_array();
	}
	
	public function get_cart_product_options( $product_id = '') {
		$select_qry = "SELECT o.product_option_id, option_type_id, product_option_name FROM  shopsy_product_options o WHERE o.product_id = " . $product_id . " ORDER BY option_seq_id ASC ";
		$opt_res = $this->ExecuteQuery($select_qry);
		return $opt_res->result_array();
	}
	public function get_product_shipping( $product_id = '' ) {
		...
		$this->db->select( "product_id, code, description, ship_price, next_item_price" );
		$this->db->from( 'shopsy_product_shipping' );
		$this->db->where('product_id', $product_id );
		$query = $this->db->get(); 
		if ( $query->num_rows() ) {
	...
	public function getPromotionsCount($promo_current = true) {
		...
		$sql = "SELECT count(*) as total_promotions FROM saa_shop_promotions where user_id = " . $user_id . " AND status = 1  "  ;
		$query = $this->db->query( $sql );
		$row   = $query->row_array();
		$total_active_promotions = $row['total_promotions'];
		
		$sql = "SELECT count(*) as old_promotions FROM saa_shop_promotions where user_id = " . $user_id . " AND status = 0  LIMIT 10 "  ;
		$query = $this->db->query( $sql );
		$row   = $query->row_array();
		$total_past_promotions =  $row['old_promotions'];
		$count =  array(
		           'current_promotions' => $total_active_promotions,
		           'past_promotions' => $total_past_promotions
				   );
	    $promotions = array();
		if( $promo_current && $total_active_promotions > 0 ) {
			$where=array('pr.status'=> '1' , 'pr.user_id' => $user_id);
		} else {
			$where=array('pr.status'=> '0' , 'pr.user_id' => $user_id);
		}
		$this->db->select('p.product_name,p.seourl, p.price, p.image,pr.*');
		$this->db->from('saa_shop_promotions as pr');
	...
	public function getProductPromotion( $product ) {
		...
		$this->db->select('p.product_name,p.seourl, p.price, p.image,pr.*');
		$this->db->from('saa_shop_promotions as pr');
		$this->db->join(PRODUCT.' as p','p.id = pr.product_id AND p.seourl = ' . $this->db->escape($product) );
		$where=array('pr.status'=> '1' , 'pr.user_id' => $user_id);
		//$this->db->join(SELLER.' as s','s.seller_id= p.user_id','left');
		$this->db->where($where);
		//$this->db->group_by('p.status');
	...
	public function getPromotionsByID( $product_id ) {
		$this->db->select('pr.*');
		$this->db->from('saa_shop_promotions as pr');
		$where=array('pr.status'=> '1' , 'pr.product_id' => $product_id, 'pr.promotion_type' => 'product' );
		$this->db->where($where);
		$result = $this->db->get();
		
		return $result->row_array();
	}
	public function getStorePromotions( $shop_id ) {
		$this->db->select('pr.*');
		$this->db->from('saa_shop_promotions as pr');
		$where=array('pr.status'=> '1' , 'pr.shop_id' => $shop_id, 'pr.promotion_type' => 'store' );
		$this->db->where($where);
		$result = $this->db->get();
	...
	public function getProductID($product)	{
		...
		$this->db->select('id');
		$this->db->from(PRODUCT);
		$where=array('status'=> 'Publish' , 'user_id' => $user_id, 'seourl' => $product);
		$this->db->where($where);
		$result = $this->db->get();
	...
	public function get_main_category() {
		return $this->db->select('id,cat_name,rootID, seourl')
						->from('shopsy_category')
	...
	public function getRelatedProducts( $category_id ) {
		...
		$where=array('status'=> 'Publish' , 'category_id' => $category_id);
		$result = $this->db->select('id, product_name, seourl, price, image')->from(PRODUCT)->where($where)->limit(12)->get();
